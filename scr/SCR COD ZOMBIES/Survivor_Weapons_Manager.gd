...
func Load_Projectile(_spread):
	#animation_player_corpo.queue("Armature|Corpo_atirando")
...
func _on_pick_up(_weapon_name, _current_ammo, _reserve_ammo, TYPE, Pick_Up_Ready):
	...
	if Weapon_In_Stack != -1:
		# Caso a arma já esteja na pilha, adiciona munição
		var remaining = Add_Ammo(_weapon_name, _current_ammo + _reserve_ammo)
		...
		if Pick_Up_Ready == true:
			# Remove a arma atual do WeaponStack
			var current_weapon_index = WeaponStack.find(Current_Weapon.Weapon_Name)
			if current_weapon_index != -1:
				WeaponStack.pop_at(current_weapon_index)

			# Ajusta GetRef para garantir que está dentro dos limites de WeaponStack
			var GetRef = WeaponStack.find(Current_Weapon.Weapon_Name)
			if GetRef == -1:
				GetRef = WeaponStack.size()  # Adiciona ao final da lista se não encontrado

			# Insere a nova arma no WeaponStack
			WeaponStack.insert(GetRef, _weapon_name)

			# Zera a munição da nova arma
			Weapon_List[_weapon_name].Current_Ammo = _current_ammo
			Weapon_List[_weapon_name].Reserve_Ammo = _reserve_ammo

			# Atualiza o WeaponStack e troca para a nova arma
			Update_Weapon_Stack.emit(WeaponStack)
			exit(_weapon_name)
